...
export class WebAudioSlider extends WebAudioControlsWidget {
  constructor() {
    super();
  }
  connectedCallback() {
...
`;
  ...
  setupImage() {
    ...
    if (this.src == null || this.src == "") {
      ...
      const svgbody = `<svg xmlns="http://www.w3.org/2000/svg" width="${
        this.sw
      }" height="${this.sh}" preserveAspectRatio="none">
<defs>
  <filter id="f1">
    <feGaussianBlur in="SourceGraphic" stdDeviation="0.8" />
  </filter>
...
<rect x="1" y="1" rx="${r}" ry="${r}" width="${this.sw - 2}" height="${
        this.sh - 2
      }" fill="#000"/>
<rect x="3" y="3" rx="${r}" ry="${r}" width="${this.sw - 6}" height="${
        this.sh - 6
      }" fill="${this.coltab[1]}" filter="url(#f1)"/>
<rect x="1" y="1" rx="${r}" ry="${r}" width="${this.sw - 2}" height="${
        this.sh - 2
      }" fill="url(#g1)"/>
</svg>`;
    ...
    this.bodyimg.onload = () => {
      ...
      if (this._knobsrc == null) {
        ...
        const svgknob = `<svg xmlns="http://www.w3.org/2000/svg" width="${
          this.kw
        }" height="${this.kh}" preserveAspectRatio="none">
<defs>
  <filter id="f1">
    <feGaussianBlur in="SourceGraphic" stdDeviation="0.8" />
  </filter>
...
<rect x="2" y="2" width="${this.kw - 4}" height="${
          this.kh - 4
        }" rx="${mm}" ry="${mm}" fill="#000"/>
<rect x="3" y="3" width="${this.kw - 6}" height="${
          this.kh - 6
        }" rx="${mm}" ry="${mm}" fill="url(#g1)"/>
<rect x="6" y="6" width="${kw2}" height="${kh2}" rx="${mm}" ry="${mm}" fill="url(#g2)" filter="url(#f1)"/>
<rect x="3" y="3" width="${this.kw - 6}" height="${
          this.kh - 6
        }" rx="${mm}" ry="${mm}" fill="url(#g3)"/>
</svg>`;
  ...
  redraw() {
    ...
    if (this.dr == "horz") {
      ...
      style.top =
        (this.sh - this.kh - this.dlen) * 0.5 + (1 - ratio) * this.dlen + "px";
      this.sensex = 0;
      this.sensey = 1;
    }
  }
...
try {
  customElements.define("webaudio-slider", WebAudioSlider);
} catch (error) {
  console.log("webaudio-slider already defined");
}

export class WebAudioSwitch extends WebAudioControlsWidget {
  constructor() {
    super();
  }
  connectedCallback() {
...
`;
  ...
  setupImage() {
    this.coltab = this.colors.split(";");
    this.kw =
      this._width || this._diameter || opt.switchWidth || opt.switchDiameter;
    this.kh =
      this._height || this._diameter || opt.switchHeight || opt.switchDiameter;
    this.img = new Image();
    this.srcurl = null;
    if (this.src == null || this.src == "") {
      ...
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${
        this.kw
      }" height="${this.kh * 2}" preserveAspectRatio="none">
<defs>
<linearGradient id="g1" x1="0%" y1="0%" x2="0%" y2="100%">
  <stop offset="0%" stop-color="#000" stop-opacity="0"/>
...
<g id="p1">
  <rect x="${kw * 0.075}" y="${kh * 0.075}" width="${kw * 0.85}" height="${
        kh * 0.85
      }" rx="${mm * 0.1}" ry="${mm * 0.1}" fill="#000"/>
  <rect x="${kw * 0.1}" y="${kh * 0.1}" width="${kw * 0.8}" height="${
        kh * 0.8
      }" rx="${mm * 0.1}" ry="${mm * 0.1}" fill="${this.coltab[1]}"/>
</g>
<g id="p2">
  <circle cx="${kw * 0.5}" cy="${kh * 0.5}" r="${
        mm * 0.35
      }" stroke="#000" stroke-width="${mm * 0.03}" fill="${
        this.coltab[0]
      }" filter="url(#f1)"/>
  <circle cx="${kw * 0.5}" cy="${kh * 0.5}" r="${
        mm * 0.27
  ...
  <rect x="${kw * 0.075}" y="${kh * 0.075}" width="${kw * 0.85}" height="${
        kh * 0.85
      }" rx="${mm * 0.1}" ry="${mm * 0.1}" fill="url(#g1)"/>
</g>
<use href="#p1" y="${kh}"/>
<use href="#p2" y="${kh}"/>
<circle cx="${kw * 0.5}" cy="${kh * 1.5}" r="${
        mm * 0.25
      }" fill="url(#g2)" filter="url(#f1)"/>
...
try {
  customElements.define("webaudio-switch", WebAudioSwitch);
} catch (error) {
...
try {
  customElements.define("webaudio-param", WebAudioParam);
} catch (error) {
  console.log("webaudio-param already defined");
}

export class WebAudioKeyboard extends WebAudioControlsWidget {
  constructor() {
    super();
  }
  connectedCallback() {
...
`;
    ...
    this.press = 0;
    this.keycodes1 = [
      90, 83, 88, 68, 67, 86, 71, 66, 72, 78, 74, 77, 188, 76, 190, 187, 191,
      226,
    ];
    this.keycodes2 = [
      81, 50, 87, 51, 69, 82, 53, 84, 54, 89, 55, 85, 73, 57, 79, 48, 80, 192,
      222, 219,
    ];
      ...
      for (let i = 0; i < p.length; ++i) {
        ...
        if (py >= 0 && py < this.height) {
          if (py < this.bheight) {
            ...
            k += ko;
            k =
              this.min + ((k / 7) | 0) * 12 + this.kn[k % 7] - this.kn[ko % 7];
          }
...
try {
  customElements.define("webaudio-keyboard", WebAudioKeyboard);
} catch (error) {
